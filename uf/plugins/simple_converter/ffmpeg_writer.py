...
class FFMPEG_VideoWriter:
    ...
    def __init__(self, filename, size, fps, codec="libx264", audiofile=None, preset="medium", bitrate=None,
        ...
        # order is important
        cmd0 = ['ffmpeg', '-y', '-loglevel', 'error' if logfile == sp.PIPE else 'info', '-f',
            'rawvideo', '-vcodec', 'rawvideo', '-s', '%dx%d' % (size[0], size[1]), '-pix_fmt',
            'rgba' if withmask else 'rgb24', '-r', '%.02f' % fps, '-i', '-', '-an', ]
        cmd = 'ffmpeg -y -loglevel {loglevel} -f rawvideo -pix_fmt rgb24 -s {size} -i - -c:v libvpx-vp9 -crf 32 -b:v ' \
              '0 -deadline realtime -cpu-used -8 -an'.format(loglevel='error' if logfile == sp.PIPE else
        'info', size='{}x{}'.format(size[0], size[1])).split()
        ...
        try:
            ...
            if b"Unknown encoder" in ffmpeg_error:
                ...
                error = error + ("\n\nThe video export "
                                 ...
                                 "video codec.") % (self.codec, self.ext)
...
def ffmpeg_write_video(clip, filename, fps, codec="libx264", bitrate=None, preset="medium", withmask=False,
                       write_logfile=False, audiofile=None, verbose=True, threads=None, ffmpeg_params=None,
                       progress_bar=True):
    ...
    with FFMPEG_VideoWriter(filename, clip.size, fps, codec=codec, preset=preset, bitrate=bitrate, logfile=logfile,
                            audiofile=audiofile, threads=threads, ffmpeg_params=ffmpeg_params) as writer:

        nframes = int(clip.duration * fps)

        for t, frame in clip.iter_frames(progress_bar=progress_bar, with_times=True, fps=fps, dtype="uint8"):
            if withmask:
...
def ffmpeg_write_image(filename, image, logfile=False):
    ...
    cmd = ['ffmpeg', '-y', '-s', "%dx%d" % (image.shape[:2][::-1]), "-f", 'rawvideo', '-pix_fmt',
